...
- `ingestion/` owns fixture-only contact snapshot diffing, event-provider abstractions, and ingestion fixtures. Keep real provider adapters as future interfaces until a goal explicitly adds them.
...
- Do not read real Contacts or calendars from normal tests, builds, evals, fixtures, or agent runs. Real Contacts access is allowed only through the explicit `npm run ingest:contacts:smoke -- --name Friendy-<number>` command.
