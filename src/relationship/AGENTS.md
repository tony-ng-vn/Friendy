...
- `ingestion/` owns contact snapshot diffing, event-provider abstractions, fixture ingestion checks, and the explicit local macOS Contacts/Calendar checker. Real provider adapters are allowed only behind explicit local commands, not normal agent runs or tests.
...
- Do not read real Contacts or calendars from normal tests, builds, evals, fixtures, or agent runs. Real Contacts/Calendar access is allowed only through explicit commands such as `npm run ingest:contacts:smoke -- --name Friendy-<number>` and `npm run ingest:local:check`.
